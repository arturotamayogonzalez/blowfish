...
int Blowfish_main() {
    ...
    int type;
    printf("Teclea [1] para encriptar..\n");
    printf("Teclea [2] para desencriptar..\n");
    scanf("%d",&type);
    ...
    if(keylen < 4 || keylen > 56) {    /*Argv[1] is my key length 32-448 bits */
        perror("La llave debe ser mas grande que 4 bytes y menor que 56 bytes: ");
        return -1;
    }
    /**********************Strong key***************************************/
        FILE *fp1;
        ...
        fp1 = fopen(input_file_name, "r");
            ...
            if(type==1) {
                ...
                while(!feof(fp1)) {
                    ...
                    //printf("\t\rEn progreso %d%%", (int)((16 * percent++ * 100) / (filelen - 8)));
                    fflush(stdout);
                }
                end = clock();
                time_spent = (double)((end-begin)/CLOCKS_PER_SEC);
                FILE *peso;
                peso=fopen(input_file_name,"r");
                fseek(peso, 0L, SEEK_END);
                printf("\n*************************************\n");
                printf("Blowfish tardó %f segundos en encriptar un archivo de %lo bytes\n", time_spent, ftell(peso));
                printf("*************************************\n");
            }
            else if(type==2) {
                ...
                fseek(peso, 0L, SEEK_END);
                printf("\n*************************************\n");
                printf("Blowfish tardó %f segundos en desencriptar un archivo de %lo bytes\n", time_spent, ftell(peso));
                printf("*************************************\n");
            }
            ...
            printf("\n");
            if(type==1)
                printf("\tTus datos fueron encriptados exitosamente en %s\n", output_file_name);
            else{
                printf("\tTus datos fueron desencriptados correctamente en %s\n",output_file_name);
            break;}
        }
        return 0;
}
